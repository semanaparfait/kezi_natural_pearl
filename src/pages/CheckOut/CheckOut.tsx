...
function CheckOut() {
    ...
    const [selectedOption, setSelectedOption] = useState(1); // 1: Delivery, 2: Pickup
    const [selectedAddressId, setSelectedAddressId] = useState<string | null>(null);
    const [showNewAddressForm, setShowNewAddressForm] = useState(!token);
    const [guestAddressSnapshot, setGuestAddressSnapshot] = useState<any>(null);

    const { data: cartData } = useGetCartItemsQuery();
    const { data: getAddresses } = useGetAddressesQuery(undefined, { skip: !token });
    ...
    // Sync user data to form when it loads
    useEffect(() => {
        if (currentUser) {
            setFormData(prev => ({
                ...prev,
    ...
    const handleCheckout = async (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        // If Store Pickup is selected, skip address validation
        if (selectedOption === 2) {
            setCurrentStep(2);
            return;
        }
        ...
        if (token) {
            ...
            const hasSelected = !!selectedAddressId;
            const hasFilledNew = showNewAddressForm && formData.fullName && formData.phoneNumber && formData.addressLine1;

            if (!hasSelected && !hasFilledNew) {
                    ...
                    {currentStep === 1 && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center justify-between px-2">
                                        <h3 className="text-xl font-serif italic text-(--primary)">
                                            {token ? "Saved Addresses" : "Shipping Details"}
                                        </h3>
                                        {token && (
                                            <button 
                                            ...
                                            {getAddresses?.map((address: any) => {
                                                ...
                                                return (
                                                    <div key={address.id} onClick={() => handleSelectAddress(address.id)}
                                                        ...
                                                        <button onClick={(e) => { e.stopPropagation(); handleDeleteAddress(address.id); }} disabled={isDeleting} className="absolute bottom-6 right-6 p-2 rounded-lg bg-red-50 text-red-500 transition-all hover:bg-red-100 disabled:opacity-50">
                                                            <Trash2 size={16} />
                                                        </button>
                            ...
                            <div className="pt-10 flex justify-between items-center">
                                ...
                                <button onClick={handleCheckout} className="px-14 py-5 rounded-full bg-(--primary) text-white text-[11px] uppercase tracking-[0.2em] font-black shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3">
                                    Continue to Payment
                                </button>
                            </div>
                        </div>
                        ...
                        <Payment
                            ...
                            addressData={
                                ...
                                : selectedAddressId 
                                    ...
                                    : { 
                                        shippingAddressSnapshot: token ? { ...formData, country, state, city } : guestAddressSnapshot,
                    ...
                    <div className="bg-white rounded-[3rem] p-10 border border-gray-100 shadow-2xl sticky top-10">
                        ...
                        <div className="space-y-5 text-sm mb-10 pt-6 border-t border-gray-100">
                            <div className="flex justify-between text-gray-500 font-medium">
                                <span>Subtotal</span>
                                <span className="text-[var(--primary)] font-black">{cartData?.items?.reduce((sum: number, item: any) => sum + item.totalPrice, 0).toLocaleString() || 0} RWF</span>
                            </div>
                            ...
                            <div className="flex justify-between font-serif italic text-(--primary) pt-8 border-t text-3xl">
                                <span>Total</span>
                                <span className="font-bold not-italic">{cartData?.items?.reduce((sum: number, item: any) => sum + item.totalPrice, 0).toLocaleString() || 0} RWF</span>
                            </div>
                        </div>
