...
const Invoice = () => {
  // Mock data for the "Classic & Professional" feel
  const invoiceData = {
    id: "#INV-2026-0882",
        ...
        <div className="p-8 md:p-16">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-16">
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="text-sm text-gray-400 mt-2 space-y-1">
                  <p className="flex items-center gap-2"><MapPin size={12} /> 77 Business Square, Kigali</p>
                  <p className="flex items-center gap-2"><Globe size={12} /> www.luxeagency.com</p>
                </div>
              </div>
            </div>
            ...
            <div className="text-right space-y-2">
              ...
              <h3 className="text-4xl font-serif italic text-gray-300">{invoiceData.id}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
            <div>
              ...
              <h4 className="text-lg font-bold text-[var(--primary)]">{invoiceData.client.name}</h4>
              <p className="text-gray-500 text-sm mt-1">{invoiceData.client.address}</p>
              <p className="text-gray-500 text-sm">{invoiceData.client.email}</p>
            </div>
            <div className="md:text-right">
              <p className="text-[10px] uppercase tracking-[0.2em] font-black text-[var(--gold-color)] mb-4">Payment Terms</p>
              <h4 className="text-lg font-bold text-[var(--primary)]">Net 15 Days</h4>
              ...
              <tbody className="divide-y divide-gray-50">
                {invoiceData.items.map((item, index) => (
                  <tr key={index} className="group hover:bg-gray-50/50 transition-colors">
                    <td className="py-6 font-bold text-[var(--primary)]">{item.name}</td>
                    <td className="py-6 px-4 text-center text-gray-500">{item.qty}</td>
                    <td className="py-6 px-4 text-right text-gray-500">${item.price.toLocaleString()}</td>
                    <td className="py-6 text-right font-bold text-[var(--primary)]">${(item.price * item.qty).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="mt-12 flex justify-end">
            <div className="w-full md:w-64 space-y-3">
              ...
              <div className="pt-4 border-t border-gray-100 flex justify-between items-center">
                ...
                <span className="text-2xl font-black text-[var(--gold-color)]">${total.toLocaleString()}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-20 p-8 bg-[var(--secondary-cream-white)] rounded-[2rem] border border-[var(--gold-color)]/10 text-center">
            ...
            <p className="text-xs text-gray-400 mt-2 max-w-sm mx-auto leading-relaxed">
              If you have any questions concerning this invoice, please contact our support at <span className="font-bold">support@luxeagency.com</span>
            </p>
          </div>
        </div>
      </div>
