...
function Shop() {
  ...
  if (prodLoading) return <div className="min-h-screen flex items-center justify-center italic font-serif text-(--primary)">Preparing the collection...</div>;
    ...
    <div className="min-h-screen bg-[var(--secondary-cream-white)] text-gray-800">
      ...
      <header className="relative py-24 px-6 overflow-hidden border-b border-gray-100 bg-white">
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          <h1 className="text-5xl md:text-7xl font-serif text-(--primary) mb-6 tracking-tight italic">The Collection</h1>
          ...
          <aside className="lg:w-72 space-y-12 md:sticky top-24 self-start">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full bg-white border border-gray-200 rounded-2xl py-4 px-12 text-sm outline-none focus:ring-2 focus:ring-(--primary)/5 transition-all shadow-sm"
              />
            ...
            <div>
              <div className="flex justify-between items-end mb-6">
                <h2 className="text-[11px] uppercase tracking-[0.2em] font-black text-(--primary)">Collections</h2>
                ...
                {["All", ...(categoriesData?.map(c => c.name) || [])].map((cat) => (
                  <label key={cat} className="group flex items-center justify-between cursor-pointer">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div className={`w-4 h-4 rounded-full border border-gray-300 flex items-center justify-center transition-all ${selectedCategory === cat ? 'border-(--primary) bg-(--primary)' : 'group-hover:border-(--primary)'}`}>
                        {selectedCategory === cat && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                      </div>
                      <span className={`text-sm tracking-tight transition-colors ${selectedCategory === cat ? 'text-black font-bold' : 'text-gray-500 group-hover:text-(--primary)'}`}>
                        {cat}
                      </span>
                    </div>
                  </label>
            ...
            {/* Price Filter */}
            <div>
              <h2 className="text-[11px] uppercase tracking-[0.2em] font-black text-(--primary) mb-6">Price Range</h2>
              <div className="space-y-3">
                {[
                  ...
                  <label key={range.val} className="group flex items-center gap-3 cursor-pointer">
                    <input
                      ...
                      className="peer hidden"
                    />
                    <div className={`w-4 h-4 rounded-sm border border-gray-300 flex items-center justify-center transition-all ${priceRange === range.val ? 'bg-(--primary) border-(--primary)' : 'group-hover:border-(--primary)'}`}>
                      {priceRange === range.val && <Check size={12} className="text-white" />}
                    </div>
                    <span className={`text-sm transition-colors ${priceRange === range.val ? 'text-black font-bold' : 'text-gray-500 group-hover:text-(--primary)'}`}>
                      {range.label}
                    </span>
                  </label>
                ))}
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-12 pb-4 border-b border-gray-100">
              <p className="text-[10px] text-gray-400 uppercase tracking-[0.2em] font-bold">
                Showing <span className="text-(--primary)">{sortedProducts.length}</span> Treasures
              </p>
            ...
            {currentProducts.length === 0 ? (
              <div className="py-32 text-center">
                ...
                <button onClick={resetFilters} className="mt-4 text-[10px] uppercase tracking-widest font-bold text-(--primary) hover:underline">Clear all filters</button>
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-16">
                {currentProducts.map(product => {
                  ...
                  return (
                    ...
                    >
                      <div className="relative aspect-[4/5] overflow-hidden rounded-[2rem] bg-white border border-gray-100 transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-(--primary)/5 group-hover:-translate-y-1">
                        {isOutOfStock && (
                        ...
                        />
                        <button className="absolute top-4 right-4 p-2.5 rounded-full bg-white/80 backdrop-blur-md text-(--primary) opacity-0 group-hover:opacity-100 transition-all hover:bg-(--primary) hover:text-white shadow-sm">
                          <Heart size={16} />
                        </button>
                      </div>
                      ...
                      <div className="mt-6 px-2 space-y-2">
                        ...
                        <h3 className="font-serif text-lg text-(--primary) leading-tight group-hover:text-black transition-colors">{product.name}</h3>
                        <p className="text-sm font-bold text-gray-900">{product.price?.toLocaleString()} RWF</p>
                        
                        <button className={`w-full mt-4 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${isOutOfStock ? 'bg-gray-100 text-gray-400' : 'bg-(--primary) text-white shadow-lg shadow-(--primary)/10 hover:brightness-110'}`}>
            ...
            {pageCount > 1 && (
              <div className="mt-24 flex justify-center">
                <ReactPaginate
                  ...
                  pageLinkClassName="w-10 h-10 flex items-center justify-center rounded-full border border-gray-100 text-[10px] font-bold hover:bg-(--primary) hover:text-white transition-all"
                  activeLinkClassName="!bg-(--primary) !text-white !border-(--primary) shadow-lg"
                  previousLinkClassName="text-[10px] font-black tracking-widest text-gray-400 hover:text-(--primary)"
                  nextLinkClassName="text-[10px] font-black tracking-widest text-gray-400 hover:text-(--primary)"
                />
              </div>
            )}
          </div>
      ...
      {notifyEmailModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-(--primary)/20 backdrop-blur-md animate-in fade-in duration-300">
           <div className="bg-white rounded-[3rem] p-12 max-w-sm w-full text-center relative shadow-2xl">
              ...
              <h2 className="text-2xl font-serif italic text-(--primary) mb-2">Join the Ritual</h2>
